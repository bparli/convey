...
use pnet::datalink::{linux, DataLinkSender, DataLinkReceiver};
use pnet::packet::arp::{ArpHardwareTypes, ArpPacket, ArpOperations, MutableArpPacket};
...
impl Arp {
    pub fn new(interface: NetworkInterface, addr: Ipv4Addr) -> Option<Arp> {
        ...
        let (iface_tx, iface_rx) =
        match linux::channel(&interface, Default::default()) {
        ...
        if let IpNetwork::V4(ipv4_net) = local_net {
            ...
            l2_listen(iface_rx, arp_cache.clone());
            ...
            boot_arp_table(arp_cache.clone(), l2_broadcast_tx.clone(), addr);
...
fn l2_listen(mut iface_rx: Box<dyn DataLinkReceiver>, mut arp_cache: Arp) {
 ...
 fn l2_broadcast_arp(mut iface_tx: Box<dyn DataLinkSender>, rx: Receiver<Ipv4Addr>, mac_addr: MacAddr, local_ip: Ipv4Addr) {
    thread::spawn(move || loop {
        match rx.recv() {
            Ok(ipv4_addr) => {
...
fn boot_arp_table(mut arp_cache: Arp, tx: Sender<Ipv4Addr>, ip: Ipv4Addr){
    loop {
        match tx.send(ip) {
            Ok(_) => {},
            Err(e) => {
        ...
        if let Some(_mac) = arp_cache.get_default_mac() {
            break;
        }
