...
impl Server {
    ...
    pub fn run(&mut self, sender: Sender<StatsMssg>) {
        for lb in self.lbs.iter() {
            ...
            // spawn each load balancer on a native thread
...
fn process_packet(
    ...
    match ether.get_ethertype() {
        EtherTypes::Arp => arp_cache.handle_arp(&ether),
        EtherTypes::Ipv4 => {
            match Ipv4Packet::new(ether.payload()) {
                Some(ip_header) => {
                            ...
                            Some(mut tcp_header) => {
                                if tcp_header.get_destination() == lb.listen_port {
                                    if let Some(stats_update) =
                                        lb.client_handler(&ip_header, &mut tcp_header, tx.clone())
                                    {
                                    ...
                                    match client_addr {
                                        Some(client_addr) => {
                                            // drop the lock!
                                            let cli_socket =
                                                &SocketAddr::new(client_addr.ip, client_addr.port);
                                            std::mem::drop(guard);
                                ...
                                match sender.send(stats) {
                                    Ok(_) => {}
                                    Err(e) => {
                                        error!("Error sending stats message on channel: {}", e)
                                    }
                                }
                            }
                            None => {}
...
pub fn run_server(lb: &mut LB, sender: Sender<StatsMssg>) {
    ...
    let (outgoing_tx, outgoing_rx) = may::sync::mpsc::channel();

    let thread_lb = lb.clone();
    let thread_sender = sender.clone();
    let thread_arp_cache = arp_cache.clone();
    go!(move || {
        loop {
            match iface_rx.next() {
                Ok(packet) => {
                    let ethernet = EthernetPacket::owned(packet.to_vec()).unwrap();
                    let mut proc_lb = thread_lb.clone();
                    let proc_tx = outgoing_tx.clone();
                    let proc_sender = thread_sender.clone();
                    let mut proc_arp_cache = thread_arp_cache.clone();
                    go!(move || {
                        process_packet(
                            &mut proc_lb,
                            ethernet,
...
mod tests {
    ...
    use self::passthrough::{find_interface, process_packet};
    ...
    fn test_new_passthrough() {
        ...
        let (tx, _) = may::sync::mpsc::channel();
    ...
    fn test_passthrough_process_packets() {
        ...
        process_packet(
            &mut thread_lb,
            EthernetPacket::owned(test_eth.packet().to_owned()).unwrap(),
            outgoing_tx.clone(),
            stats_tx.clone(),
            &mut arp_cache,
        );
